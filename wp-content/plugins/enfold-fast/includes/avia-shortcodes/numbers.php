...
if ( ! class_exists( 'ep_sc_animated_numbers' ) ) 
{
	
	class ep_sc_animated_numbers extends aviaShortcodeTemplate
	{
		/**
		...
		function shortcode_insert_button()
		{
			...
			$this->config['shortcode']		= 'ep_animated_numbers';
			...
			$c = array(
						...
						array(	
							...
							'subtype'	=> array(
												...
												__( 'Yes, display an icon in front of number (outside)', 'avia_framework' )	=> 'av-icon-before-outside',	
												__( 'Yes, display an icon after the number (outside)', 'avia_framework' )	=> 'av-icon-after-outside',	
											)
						),	
		...
		function shortcode_handler( $atts, $content = '', $shortcodename = '', $meta = '' )
		{
			...
			$display_char = $before = $after = $before_outside = $after_outside = '';
			...
			if( $icon_select !== 'no' )
			{
				...
				if( $icon_select == 'av-icon-before-outside' ) 
				{
					$before_outside = $display_char;
				}
				if( $icon_select == 'av-icon-after-outside' ) 
				{
					$after_outside = $display_char;
				}
			}
			...
			$output  = '<' . $tags[0] . ' ' . $meta['custom_el_id'] . ' class="ep-animated-number av-force-default-color ' . $av_display_classes . ' avia-color-' . $color . ' ' . $meta['el_class'] . '" ' . $style . ' data-timer="' . $timer . '" data-lazy="' . ENFOLD_FAST_ASSETS .'js/numbers.js" ' . $wrapper_data . '>';
			$output .= $circle_markup;


			$output .=		$before_outside;
			$output .= 		'<strong class="heading" ' . $font_style . '>';
			$output .= 		$before . $this->extract_numbers( $number, $number_format, $atts ) . $after;
			$output .= 		'</strong>';
			$output .= 		$after_outside;
			$output .= 		"<div class='ep-animated-number-content' {$font_style2}>";
		...
		protected function extract_numbers( $number, $number_format, &$atts )
		{
			...
			$replace = '<span class="ep-single-number number-rollup __av-single-number" data-number-rollup-end="$1" data-number-rollup-start="' . $atts['start_from'] . '" data-number-rollup-duration="1700">$1</span>';

			$number = preg_replace( '!(\D+)!', '<span class="ep-no-number">$1</span>', $number );
