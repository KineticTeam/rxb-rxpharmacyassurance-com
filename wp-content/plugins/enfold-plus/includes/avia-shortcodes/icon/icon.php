...
if ( ! class_exists( 'av_font_icon' ) )
{
    class av_font_icon extends aviaShortcodeTemplate
    {
        ...
        function extra_assets()
		{
			...
			wp_enqueue_style( 'avia-module-ep-icon' , ENFOLD_PLUS_ASSETS . 'css/ep_icon.css' , array( 'avia-module-icon' ), ENFOLD_PLUS_VERSION, false );
		}
					...
					array(
						...
						array(	
							...
							array(
								...
								"type" 	=> "select",
								"lockable" => true,
								"std" 	=> apply_filters( "avf_ep_icon_style_std", "" ),
								"subtype" => apply_filters( "avf_ep_icon_style_options", array(
		...
		protected function register_dynamic_templates()
		{
			...
			$c = array(
						...
						array(	
							'name' 	=> __( 'Icon Position (Tablet)', 'avia_framework' ),
							'desc' 	=> __( 'Choose the alignment of your icon here', 'avia_framework' ),
							'id' 	=> 'tablet_position',
							'type' 	=> 'select',
							'std' 	=> '',
						...
						array(	
							'name' 	=> __( 'Icon Position (Mobile)', 'avia_framework' ),
							'desc' 	=> __( 'Choose the alignment of your icon here', 'avia_framework' ),
							'id' 	=> 'mobile_position',
							'type' 	=> 'select',
							'std' 	=> '',
        ...
        function shortcode_handler( $atts, $content = '', $shortcodename = '', $meta = '' )
        {
			...
			$tablet_position = isset( $atts['tablet_position'] ) ? "avia-icon-pos-".$atts['tablet_position'] : "";
			$mobile_position = isset( $atts['mobile_position'] ) ? "avia-icon-pos-".$atts['mobile_position'] : "";
			...
			$wrapper_data = apply_filters( "avf_ep_icon_wrapper_data", "", $meta );
			$custom_class .= ' ' . $meta['ep_class'];

            $display_char = "<{$tags[0]} class='av-icon-char' style='{$size_string}' {$char} {$tooltip} {$wrapper_data}></{$tags[1]}>";
            
            $output = '<span ' . $meta['custom_el_id'] . ' class="' . $shortcodename . ' avia_animate_when_visible' . $animation_class . $av_display_classes . ' av-icon-style-' . $style . ' ' . $custom_class . ' avia-icon-pos-' . $position . ' ' . $tablet_position . ' ' . $mobile_position . '" style="' . $color . '">' . $display_char.$caption . '</span>';
