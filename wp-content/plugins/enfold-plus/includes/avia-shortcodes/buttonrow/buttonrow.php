...
if ( ! class_exists( 'avia_sc_buttonrow' ) ) 
{
	class avia_sc_buttonrow extends aviaShortcodeTemplate
	{
		...
		protected $spacing_unit;
		 ...
		 /**
         * 
		 * Added by Enfold Plus
		 * @since 1.0.0
		 * @var string
		 */
		protected $anim_class;
		...
		public function extra_assets()
		{
			...
			wp_enqueue_style( 'avia-module-ep-buttonrow' , ENFOLD_PLUS_ASSETS . 'css/ep_buttonrow.css' , array( 'avia-module-buttonrow' ), ENFOLD_PLUS_VERSION, false );
		}
					...
					array(
						...
						array(	
							...
							array(
								...
								"type" 	=> "select",
								"lockable" => true,
								"std" 	=> apply_filters( "avf_ep_buttonrow_style_std", "" ),
								"subtype" => apply_filters( "avf_ep_buttonrow_style_options", array(
		...
		protected function register_dynamic_templates()
		{
			...
			$c = array(
						...
						array(
							'name' 	=> __( 'Buttons Position (Desktop)', 'avia_framework' ),
							'desc' 	=> __( 'Choose the alignment of your buttons here (Desktop)', 'avia_framework' ),
							'id' 	=> 'alignment',
							'type' 	=> 'select',
							'std' 	=> 'center',
							'subtype'	=> array(
						...
						array(
							'name' 	=> __( 'Buttons Position (Tablet)', 'avia_framework' ),
							'desc' 	=> __( 'Choose the alignment of your buttons here (Tablet)', 'avia_framework' ),
							'id' 	=> 'tablet_alignment',
							'type' 	=> 'select',
							'std' 	=> '',
							'subtype'	=> array(
						...
						array(
							'name' 	=> __( 'Buttons Position (Mobile)', 'avia_framework' ),
							'desc' 	=> __( 'Choose the alignment of your buttons here (Mobile)', 'avia_framework' ),
							'id' 	=> 'mobile_alignment',
							'type' 	=> 'select',
							'std' 	=> '',
							'subtype'	=> array(
		...
		public function shortcode_handler( $atts, $content = '', $shortcodename = '', $meta = '' )
		{
			...
			$this->anim_class = empty( $animation ) ? "" : " av-animated-generic " . $animation . " ";

			$tablet_alignment = $tablet_alignment ? "avia-buttonrow-{$tablet_alignment}" : "";
			$mobile_alignment = $mobile_alignment ? "avia-buttonrow-{$mobile_alignment}" : "";

			$wrapper_data = apply_filters( "avf_ep_buttonrow_wrapper_data", "", $meta );

			$output = '';
			$output .=	"<div {$meta['custom_el_id']} class='avia-buttonrow-wrap avia-buttonrow-{$this->alignment} {$tablet_alignment} {$mobile_alignment} {$av_display_classes} {$meta['el_class']}' {$wrapper_data}>";
		...
		function av_buttonrow_item( $atts, $content = '', $shortcodename = '' )
		{
			...
			$output = '';
			$output .=	"<a href='{$link}' {$data} class='avia-button {$extraClass} " . $this->class_by_arguments('icon_select, color, size', $atts, true) . "' {$blank} {$style} {$title_attr} {$onclick}>";
